...
function (zm, z0=NA, umean, h, ol, sigmav, ustar, wind_dir = 0, 
...
#    skip_count   = number of observations skipped
#
#                   
# Example
...
#--------------------------------------------------------------------
    ...
    for (v in 1:length(output_list)) assign(names(output_list)[v], 
        output_list[[v]])
	  # To keep track of amount of data skipped
    skip_count <- 0
#--------------------------------------------------------------------
# Create output array
#--------------------------------------------------------------------
...
#--------------------------------------------------------------------
              ...
              print(paste0("Skipping footprint calculation: ", 
                  as.character(foot_loop), ". ", skip_count, " skipped."))
            }
        }
...
#--------------------------------------------------------------------
        ...
        if (!is.na(rs[1]) & (length(fclim_2d) > 1) & (sum(!is.na(fclim_2d)) > 1)) {
            ...
            FFP$fr  <- rs * NA
            ffp_tmp <- NULL
            f_sort  <- sort(c(fclim_2d), decreasing = TRUE)
